...
function migrateControlCenterTables(database: Database.Database): void {
  database.exec(`
    ...
    CREATE TABLE IF NOT EXISTS managed_messages (
      ...
      updated_at TEXT NOT NULL,
      PRIMARY KEY (guild_id, content_type, entity_id)
    );
    ...
    CREATE INDEX IF NOT EXISTS idx_reviews_guild
      ON reviews (guild_id, created_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_projects_guild
      ON projects (guild_id, updated_at DESC);
    ...
    CREATE INDEX IF NOT EXISTS idx_backups_guild
      ON guild_backups (guild_id, created_at DESC);
  `);
}
